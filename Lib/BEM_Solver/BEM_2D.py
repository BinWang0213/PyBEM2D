...
from .Elements.Unified_Element import build_matrix, solution_allocate, Field_Solve
from .Elements.Unified_Element_Trace import build_matrix_trace, solution_allocate_trace, Field_Solve_trace
...
class BEM2D:
    ...
    def set_Mesh(self, Pts_e=[], Pts_t=[], h_edge=0.1, h_trace=0.1, Ne_edge=None, Ne_trace=None, Type='Quad', mode=0):
        ...
        Date: July. 2017
        """
        self.Mesh.set_Mesh(Pts_e,Pts_t,h_edge,h_trace,Ne_edge,Ne_trace,Type,mode)
    ...
    def plot_Mesh(self, Annotation=1):
        ...
        Date: July. 2017
        """
        self.Mesh.plot_Mesh(Annotation)
    ...
    def SetProps(self,k_tensor,miu=1,h=1):
    ...
    def Solve(self,DDM=0,AB=[],debug=1):
        ...
        if (self.TraceOn==1):#Internal Trace ON
            ...
            Ab=build_matrix(self.BEs_edge,self.Mesh,DDM,AB) #matrix AB
            if(debug): print("[Solution] Solving problem...",end='')
            X = np.linalg.solve(Ab[0], Ab[1])#linear solution X
            if(debug): print("Done")
            solution_allocate(self.BEs_edge,self.Mesh,X,debug)#assign solution back to element class
            return Ab
    ...
    def get_Solution(self, Pts, bd_markerID=-1):
        ...
        if (self.TraceOn == 1):  #Domain have internal traces
            return Field_Solve_trace(Pts[0], Pts[1], self.BEs_edge,self.BEs_trace,self.Mesh, elementID=Pts_location)
        else:  # General case
            return Field_Solve(Pts[0], Pts[1], self.BEs_edge, self.Mesh, elementID=Pts_location)
