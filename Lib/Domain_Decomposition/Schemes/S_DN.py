            ...
            for i in range(obj.NumObj):#For each subdomain
                ...
                for j in range(Num_shared_edge):#For each connected edge in this domain
                    ...
                    if(it==0):#initial guess must be 0
                        P_new=np.zeros(len(obj.BEMobjs[i].mesh_nodes[bdID]))
                        Q_new=np.zeros(len(obj.BEMobjs[i].mesh_nodes[bdID]))
                        if(obj.BEMobjs[ConnectObjID].TypeE_edge=="Const"):
                            P_new=np.zeros(len(obj.BEMobjs[i].mesh_nodes[bdID])-1)
                            Q_new=np.zeros(len(obj.BEMobjs[i].mesh_nodes[bdID])-1)
