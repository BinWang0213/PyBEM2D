...
def PRR(obj,alpha,TOL,opt):
        ...
        for it in range(MaxIter):
            ...
            if(it>2):
                if(opt==1):
                    alpha_opt=PRR_OPT(obj,R_old_old,P_cur_old,P_con_old,Q_cur_old,Q_con_old,alpha)
                    alpha=alpha_opt
                    #print(alpha_opt)
            ...
            for i in range(obj.NumObj):#For each subdomain
                ...
                for j in range(Num_shared_edge):#For each connected edge in this domain
                    ...
                    bdID=obj.BEMobjs[i].EndPoint2bdmarker(Intersect[0],Intersect[1])
                    PQ=obj.BEMobjs[i].get_BDSolution(bdID)

                    if(it==0):
                        R_old=np.zeros(len(obj.BEMobjs[i].mesh_nodes[bdID])) #initial guess must be 0
                    ...
                    if(it>0):
                        ...
                        R_new=R_old-alpha*(P_current-P_connect+Q_current+Q_connect)
...
def PRR_OPT(obj,R_old_old,P_cur_old,P_con_old,Q_cur_old,Q_con_old,alpha_old):
