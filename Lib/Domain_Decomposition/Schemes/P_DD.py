        ...
        for it in range(MaxIter):
            ...
            for i in range(obj.NumObj):#For each subdomain
                ...
                for j in range(Num_shared_edge):#For each connected edge in this domain
                    ...
                    if(it==0):
                        P_old=np.zeros(len(obj.BEMobjs[i].mesh_nodes[bdID])) #initial guess zero vector
                        if(obj.BEMobjs[ConnectObjID].TypeE_edge=="Const"):
                            P_old=np.zeros(len(obj.BEMobjs[i].mesh_nodes[bdID])-1) #Const element number = nodes number -1
                    if(it>0):
                        P_old=PQ[0]
                    Q_current=PQ[1]
