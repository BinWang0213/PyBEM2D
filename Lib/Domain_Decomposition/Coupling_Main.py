...
from Lib.Tools.Geometry import point_on_line,Interp_Nonconforming
...
class DDM_Solver:
    ...
    def Interp_intersection(self,objID,ConnectObjID,Intersect,Vals=[],varID=0):
        ...
        bdID=self.BEMobjs[objID].EndPoint2bdmarker(Intersect[0],Intersect[1])
        CurrentNodes=self.BEMobjs[objID].mesh_nodes[bdID]
        bdID_connect=self.BEMobjs[ConnectObjID].EndPoint2bdmarker(Intersect[0],Intersect[1])
        
        if(len(Vals)==0):#Used for general variable(dlth,h,p) by solving matrix Eqs.4-6
            Val_connect=self.BEMobjs[ConnectObjID].get_BDSolution(bdID_connect)[varID]
        else:#Only Used for dltq
            Val_connect=Vals
        
        ConnectNodes=self.BEMobjs[ConnectObjID].mesh_nodes[bdID_connect]
